...
The KWin / Plasma Wayland backend (`kde_wayland_loop`) follows the same pattern: it owns a `wl_display*`, blocks on `select(wayland_fd, wake_pipe[0])`, and publishes via the same `publish_and_log()` helper. Per-window state (a 64-slot fixed array of `PlasmaSlot` records) is allocated on the listener thread's stack and never escapes — the hot path still sees only the `active_app_hash` atomic. Note: Plasma 6 KWin restricts `org_kde_plasma_window_management` to trusted clients, so the bind currently fails on a stock Plasma session; the daemon takes the KWin D-Bus bridge path described next instead. The protocol code path is preserved for older KDE versions and for wlroots-style compositors. See [KNOWN_ISSUES.md](./KNOWN_ISSUES.md).

The KWin D-Bus bridge backend (`kwin_dbus_loop`) is the Plasma 6 path. Same shape as the wayland backend — sd-bus opens the user session bus, claims `org.loginext.WindowFocus`, exposes one `Activated(s,s)` method, and `select()`s over `(sd_bus_get_fd(), wake_pipe[0])` exactly like the wayland branch does over `wayland_fd`. The two strings (`resourceClass`, `resourceName`) are FNV-1a hashed inside the method handler on the listener thread; the hot path still sees only `active_app_hash`. We must respect `sd_bus_get_events()` (POLLIN/POLLOUT mask) and `sd_bus_get_timeout()` (CLOCK_MONOTONIC absolute deadline) on every iteration — skipping either causes write stalls or missed reply timeouts. The publishing side is a tiny KWin script (`deploy/kwin/loginext-focus`) that hooks `workspace.windowActivated` and `callDBus()`s into our method on every focus change; the script is the only piece that runs inside KWin's process, so it must stay allocation-free too — one `callDBus` per focus event, no timers, no polling. Backend ordering in `thread_main()` is Hyprland → KWin D-Bus (gated on `XDG_CURRENT_DESKTOP` containing "KDE") → wayland protocol → X11, so on a KDE host the D-Bus bridge wins before the doomed protocol attempt.

If you add a new compositor backend (Sway, Mir, GNOME via libmutter), the rule is the same: the new backend lives inside `scope::listener.cpp::thread_main()` as a new branch, exits via the same `stop` atomic + self-pipe, and writes to the same `active_app_hash`. Never expose a second atomic, never lock, never let the listener call back into AppContext.

The introspection-only `Listener::intro_*` fields (mutex-protected name + source string) are written from `publish_and_log()` so the IPC `get_active_app` command can return a human-readable label. They are **never** read on the hot path — the mutex guarantees correctness for the rare on-demand IPC reader without the hot path paying any cost. If you find yourself reaching for these fields in `on_event`, you're violating rule 4 above.
...
- `get_active_app` is the only IPC command that reaches into the scope listener. It loads the atomic hash, takes the introspection mutex briefly to copy the name/source strings, and resolves the rule against the same `RuleTable` the hot path uses. It must never be called from the hot path itself — the mutex acquisition is fine for an on-demand UI request but unacceptable per-event.
