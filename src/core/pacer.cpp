...
void enqueue_combo(PacingQueue& q, const presets::KeyCombo& combo,
    ...
    // Drop no-op combos at the gate. Keeps damping math accurate (a Direction::None
    // path never reaches here, but a preset may legitimately resolve to a
    // zero-count combo for a direction it doesn't bind).
...
void process_timer(PacingQueue& q, EmitterHandle& em) noexcept {
    ...
    if (q.release_pending) {
        emit_combo_up(em, q.held_combo);
        q.release_pending = false;
        q.held_combo = {};

        // If more combos are queued, schedule the next one after pacing interval.
        if (q.count > 0) {
            arm_timer(q);
        }
        return;
    ...
    auto combo = q.buffer[q.head];
    q.head = static_cast<uint8_t>((q.head + 1) & mask);
    q.count--;

    emit_combo_down(em, combo);
