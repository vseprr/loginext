...
- **KDE Plasma 6 needs the KWin D-Bus bridge.** `kde_wayland_loop()` binds `org_kde_plasma_window_management` (v1) when `WAYLAND_DISPLAY` is set, but on Plasma 6.x KWin no longer advertises this global to regular wayland clients — only to trusted Plasma shell components. Verified on KWin 6.6.4 with `wayland-info` and a minimal C wayland client. On a KDE session (`XDG_CURRENT_DESKTOP` contains "KDE") the daemon now takes a different path entirely: `kwin_dbus_loop()` claims `org.loginext.WindowFocus` on the user session bus and waits for our KWin script (`deploy/kwin/loginext-focus`) to call `Activated(s,s)` on every `workspace.windowActivated` signal. The wayland code path stays in tree as the wlroots / older-KDE fallback (and as scaffold for the eventual `ext_foreign_toplevel_list_v1` backend). Same pattern `kdotool` uses; now shipped.
  - **The script must be enabled in System Settings → Window Management → KWin Scripts** (or `kwriteconfig6 --file kwinrc --group Plugins --key loginext-focusEnabled true && qdbus6 org.kde.KWin /KWin reconfigure`). `deploy/install.sh` does this automatically; the PKGBUILD only stages the files. If the script isn't enabled, the daemon successfully claims the bus name but receives no events — `get_active_app` returns `hash=0x00000000`, the hot path resolves everything against the global preset, and nothing crashes. Look at the daemon log for `awaiting KWin script publish` to confirm the bridge is bound but idle.
